...
def analyze(original, expanded, out):
    ...
    for repeat in range(c0["experiment"]["repeats"]):
        ...
        outside = bool(
            np.any(ct1[GAINS].to_numpy(float) > b0[:, 1] + OUTSIDE_RANGE_FRACTION * np.diff(b0)[:, 0])
        )
        row = dict(
            repeat=repeat,
            original_min_JT=float(ct0.JT),
            expanded_min_JT=float(ct1.JT),
        ...
        for name, ct, b, p in zip(("original", "expanded"), (ct0, ct1), (b0, b1), pair):
            ...
            controllers.append(
                dict(
                    arm=name,
                    repeat=repeat,
                    **{k: ct[k] for k in GAINS + OBJECTIVES},
            ...
            for budget in TRACKING_BUDGETS:
                ...
                budgets.append(
                    dict(
                        arm=name,
                        repeat=repeat,
                        JT_budget_rad=budget,
                        count=len(eligible),
                        best_observed_JE=eligible.JE.min(),
                    )
                )
    ...
    improvement = float(
        100 * (table.original_min_JT.mean() - table.expanded_min_JT.mean()) / table.original_min_JT.mean()
    )
    ...
    decision = dict(
        adopted=adopted,
        reasons=reasons,
        min_JT_improvement_pct=improvement,
        ...
        rule=dict(
            ...
            tracking_JT_max=TRACKING_JT_MAX,
            practical_JT_max=PRACTICAL_JT_MAX,
        ),
    ...
    plot_fronts(fronts, out)
    print(decision)
    return decision


def plot_fronts(fronts, out):
    """Zoom only observed regional points; never draw an edge toward the remote zero anchor."""
    ...
    for arm, f, color in zip(("Original", "Expanded"), fronts, ("C0", "C1")):
        for repeat, group in f.groupby("repeat"):
            ...
            views = [group, group.loc[group.JT <= TRACKING_JT_MAX], group.loc[group.JT <= PRACTICAL_JT_MAX]]
            for ax, view in zip(axes, views):
                ax.plot(
                    view.JT, view.JE, "o-", ms=3, alpha=0.6, color=color, label=arm if repeat == 0 else None
                )
    for ax in axes:
    ...
    for ax in axes[1:]:
        ax.set_ylim(practical_y.min() - 0.05, practical_y.max() + 0.05)
    axes[0].legend()
